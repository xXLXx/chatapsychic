...
class register extends CI_Controller {
    ...
    function index()
    {
        ...
        $this->load->view('/registration/client_register_form', $data);
    ...
    {
        $this->load->view('header');
        $this->load->view('/registration/membership_type');
        $this->load->view('footer');
    }
    ...
    function reader()
    {
        $data["error"] = $this->session->flashdata('error');
        $this->load->view('header');
        $this->load->view('/registration/reader_application_form', $data);
        $this->load->view('footer');
    }

    function reader_affiliate()
    {
        $this->load->view('header');
        $this->load->view('/registration/reader_application_form');
        $this->load->view('footer');
    }
    ...
    function reader_submit()
    {
        ...
        $this->form_validation->set_rules('ssn', 'SSN/SIN#', 'required|trim');
        $this->form_validation->set_rules('us_resident', 'Are you a U.S. resident?', 'required|trim');        
        $this->form_validation->set_rules('exp_years', 'Years of Reading Experience', 'required|trim');
        ...
        else
        {
            ...
            {
                $this->session->set_flashdata('error', "It seems like you have already registered. If you forgot your username and password you can easily retrieve it by clicking the \"Forgot Password\" link below. If you experience any further trouble, please <a href='/contact'>contact us</a>");
                echo "It seems like you have already registered. If you forgot your username and password you can easily retrieve it by clicking the \"Forgot Password\" link below. If you experience any further trouble, please <a href='/contact'>contact us</a>";
                //redirect("/register/login");
                //exit;
            }*/
            ...
            $this->upload->do_upload('article_file');
            ...
            // additionals
            $insert['address1'] = set_value('address1');
            $insert['address2'] = set_value('address2');
            $insert['city'] = set_value('city');
            $insert['state'] = set_value('state');
            $insert['zip'] = set_value('zip');
            $insert['phone'] = set_value('phone');
            $insert['has_ssn'] = set_value('ssn');
            $insert['is_us_resident'] = set_value('us_resident');
            $insert['has_gov_id'] = set_value('has_gov_id');
            $insert['member_type'] = set_value('membership');
            ...
            $data['has_friends'] = set_value('has_friends');
            $data['has_relatives'] = set_value('has_relatives');
            $data['for_testing'] = set_value('for_testing');
            $data['is_willing'] = set_value('is_willing');
            $data['writing_years'] = set_value('writing_years');
            $data['skype_id'] = set_value('skype_id');
            $data['has_paypal'] = set_value('has_paypal');
            $data['has_us_checking'] = set_value('has_us_checking');
            $data['is_gov_subsidize'] = set_value('is_gov_subsidize');            
            $data['isp'] = set_value('isp');
            $data['pc_age'] = set_value('pc_age');
            $data['article_filename'] = $file["file_name"];
            
            $device_names = array(
                "device_win" => set_value("device_win"),
                "device_mac" => set_value("device_mac"),
                "device_ipad" => set_value("device_ipad"),
                "device_ios" => set_value("device_ios"),
                "device_droid" => set_value("device_droid"),
                "device_other" => set_value("device_other"),
            );
            
            $devices = set_value('device[]');            
            $devices["divice_names"] = $device_names;
            $data['os'] = json_encode($devices);
            
            $data['en_fluent'] = set_value('en_fluent') === '' ? 'N' : 'Y';
            $data['spoken_languages'] = set_value('languages');
            $data['grammar_rate'] = set_value('com_skills');
            $data['typing_skills'] = set_value('typing_skills');
            $data['is_committed'] = set_value('is_committed');
            $data['is_wee_available'] = set_value('is_wee_available');
            $data['work_start_date'] = date("Y-m-d",strtotime(set_value('start_date')));
            $data['date_availability'] = json_encode(set_value('date_avail[]'));
            $data['areas_of_expertise'] = json_encode(set_value('expertise[]'));
            
            $divine_practices = set_value('divine_practices[]');
            $divine_practices["divine_other"] = set_value("divine_other");
            $data['divine_practices'] = json_encode($divine_practices);
            
            $data['is_clairvoyant'] = set_value('is_clairvoyant');
            $data['is_empathic'] = set_value('is_empathic');
            $data['is_clairsentient'] = set_value('is_clairsentient');
            $data['other_self'] = set_value('other_self');
            $data['method_email'] = set_value('method_email');
            $data['method_chat'] = set_value('method_chat');
            $data['method_phone'] = set_value('method_phone');
            $data['method_vidchat'] = set_value('method_vidchat');
            $data['method_sms'] = set_value('method_sms');
            $data['method_private_person'] = set_value('method_private_person');
            $data['method_radio'] = set_value('method_radio');
            $data['method_tv'] = set_value('method_tv');
            $data['method_public_venue'] = set_value('method_public_venue');            
            //$data['has_prev_readings'] = set_value('has_prev_readings');
            $data['skills'] = set_value('skills');
            $data['no_of_years'] = set_value('exp_years');
            $data['companies'] = set_value('companies');
            $data['articles'] = set_value('articles');
            $data['blogs'] = set_value('blogs');
            $data['bio'] = set_value('bio');
            $data['agree_terms'] = set_value('agree_terms');
            $data['accept_contractual'] = set_value('accept_contractual');
            $data['accept_responsibility'] = set_value('accept_responsibility');
            $data['application_status'] = set_value('status');

            $this->db->insert('member_applicants', $data);
    ...
    function checkSettings()
    {
        ...
        $applicant["dates_required"] = json_encode($applicant["dates_required"]);
        $applicant["areas_of_expertise"] = json_encode($applicant["areas_of_expertise"]);
        
        $msg = $this->applicants->checkSettings($applicant);
        if ($msg)
        {
        ...
        echo json_encode(["application_status" => "pending"]);
        exit;
    }
