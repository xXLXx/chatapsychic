...
class system_vars {
    ...
    function get_profile_subcategories($profile_id, $link_categories = true)
    {
        ...
        foreach ($getSubcategories->result_array() as $i => $p)
        {
            ...
            if (!$link_categories)
                ...
                $subcategory_string .= "<a href='/category/main/{$c['url']}'>{$c['title']}</a> / <a href='/category/sub/{$c['url']}/{$p['url']}'>{$p['title']}</a>";
    ...
    function get_categories($type = 'featured', $member_id = null)
    {
        ...
        if ($type == 'featured')
            $getPages = $CI->db->query("SELECT * FROM categories WHERE show_in_titlebar = 1 ORDER BY sort LIMIT 5 ");
        elseif ($type == 'expert')
        {
        ...
        elseif ($type == 'unregistered')
        {
            ...
            $getPages = $CI->db->query("SELECT * FROM categories ORDER BY sort");
    ...
    function get_pages($type = 'footer')
    {
        $CI = & get_instance();
        if ($type == 'footer')
            $getPages = $CI->db->query("SELECT * FROM pages WHERE add_to_footer = 1 ORDER BY sort ");
        else
            $getPages = $CI->db->query("SELECT * FROM pages  ORDER BY sort");
    ...
    function get_subuser($subUserId = null)
    {
        ...
        if ($getSubUser->num_rows() == 0)
            return false;
        else
            return $getSubUser->row_array();
    }
    ...
    function get_page($PAGEREF = '')
    {
        ...
        else
        {
            ...
            if (is_numeric($PAGEREF))
                $getP = $CI->db->query("SELECT * FROM `pages` WHERE `id`=\"{$PAGEREF}\" LIMIT 1");
            else
                $getP = $CI->db->query("SELECT * FROM `pages` WHERE `url`=\"{$PAGEREF}\" LIMIT 1");

            if ($getP->num_rows() == 0)
                return false;
            else
                return $getP->row_array();
        }
    }
    ...
    function state_array_select_box($SelectName = 'state', $SelectedItem = '', $OnChange = '')
    {
        ...
        $SlectBox = "<select style='width:auto;' class='form-control' name='{$SelectName}' $OnChange><option value=''>Select A State</option>";

        foreach ($this->state_array() as $stAb => $stNam)
        {
            $SlectBox .= "<option value='$stAb'";
            if (trim($SelectedItem) == trim($stAb))
            {
                $SlectBox .= " selected";
            } $SlectBox .= ">$stNam</option>\n";
        }
    ...
    function country_array_select_box($SelectName = 'state', $SelectedItem = 'US', $OnChange = '')
    {
        ...
        $SlectBox = "<select style='width:auto;' name='{$SelectName}' class='form-control' $OnChange><option value=''>Select A Country</option>";
    ...
    function map_address($Address = '', $City = '', $State = '', $Zip = '', $MapWidth = '', $MapHeight = '', $Zoom = '', $EnableGoogleBar = false)
    {
        ...
        if ($MapWidth)
            $config['map_width'] = $MapWidth;
        if ($MapHeight)
            $config['map_height'] = $MapHeight;
        if ($Zoom)
            $config['zoom'] = $Zoom;
        if ($EnableGoogleBar)
            $config['display_google_bar'] = TRUE;
        //
    ...
    function dob_custom($FieldName = "dob", $month_value = null, $day_value = null, $year_value = null)
    {
        ...
        $ReturnText = "
        	<table>
        		<tr>
        		<td>
	        		<select style='width:auto;' class='form-control' name='{$FieldName}_month'>
		        ...
		        <td>
		        	<select style='width:auto;' class='form-control' name='{$FieldName}_day'>
        ...
        $ReturnText .= "
        		...
        		<td>
        			<select style='width:auto;' class='form-control' name='{$FieldName}_year'>
    ...
    function omail($to, $template_name, $exchange_vars_array = array(''), $from_id = null)
    {
        ...
        else
        {
            ...
            $content = "
			<div style='background:#b9b9b9;padding:20px;'>
				<div style='margin-bottom:10px;font-size:20px;font-weight:bold;font-family:Arial;'>Chatpsychics.com</div>
				<div style='background:#FFF;padding:20px;'>{$pre_content}</div>
			</div>
			";
...
//Initializations 
    ...
    function get_side_topics()
    {
        $CI = & get_instance();
        $sql = "
            SELECT * FROM pages WHERE url IN ('apply-as-reader', 'interested-in-both')
            ";
        $q = $CI->db->query($sql);
    ...
    function show_side_topics()
    {
        $CI =& get_instance();
        echo $CI->load->view("pages/side_topic", null, true);
    }
}
