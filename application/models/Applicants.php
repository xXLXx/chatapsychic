...
class applicants extends CI_Model {
    ...
    function getInfo($member_id)
    {
        ...
        $devices = json_decode($result["os"], true);
        $result["os"] = is_array($devices) ? $devices : [];
    ...
    function validateSettings($settings, $applicant)
    {
        ...
        if (trim($settings["os_required"]) !== "")
        {
            $os_found = false;            
            
            foreach ($applicant["os"] as $os)
            {
                if (strpos(strtolower($settings["os_required"]), strtolower($os) ) !== false)
                {
                    $os_found = true;
                    break;
                }
