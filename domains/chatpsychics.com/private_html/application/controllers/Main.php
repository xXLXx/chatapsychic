...
class Main extends CI_Controller {
    ...
    function index()
    {
        ...
        $this->load->view('pages/homepage', $t);
        $this->load->view('footer');
    }

    // Joe's Edit: Added function for old page - Task #1
    // Located at http://www.chatpsychics.com/main/old_homepage
    function old_homepage()
    {

        $this->hide_banner = true;

        $t = $this->system_vars->get_page('homepage_old');

        $getArticles = $this->db->query("SELECT * FROM articles ORDER BY id DESC LIMIT 5");
        $t['articles'] = $getArticles->result_array();

        $this->load->view('header');
        $this->load->view('pages/homepage_old', $t);
        $this->load->view('footer');
    }
